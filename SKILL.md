...
├── SKILL.md                   # 本文件
├── scripts/
│   ├── serve.py               # 主入口：一站式启动（后台爬取 + 服务 + 浏览器）
│   └── crawler.py             # 数据获取模块（可独立 CLI 使用）
├── references/
...
| 功能 | 说明 |
|------|------|
| Loading 动画 | 服务启动后先展示过渡动画 + 实时爬取进度，数据就绪后自动渲染 |
...
| `references/template.html` | 前端渲染：loading 动画 + 统计卡片 + 颜色表格 + 筛选搜索 |
